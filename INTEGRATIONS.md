...
**repo-roast** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `repo-roast` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
repo-roast ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
repo-roast ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
repo-roast ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `repo-roast` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="repo-roast") for rec in records]   # records = your JSON output
